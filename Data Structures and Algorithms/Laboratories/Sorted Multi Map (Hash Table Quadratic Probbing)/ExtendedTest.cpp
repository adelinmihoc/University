...
void testRemoveFromIterator() {
    std::cout << "Test remove from iterator\n";
    SortedMultiMap smm = SortedMultiMap(asc);
    smm.add(12,4);
    smm.add(7,4);
    smm.add(19,4);
    ...
    itsmm.next(); // 12
    itsmm.next(); // 19
    assert(itsmm.remove() == std::make_pair(19, 4)); // deleted from iterator
    assert(smm.search(19).size() == 0); // deleted from map

    itsmm.remove();
    try {
        itsmm.remove();
        //assert(smm.search(22).size() == 0); // deleted from map
        assert(false);
    } catch (std::exception&) {
        assert(true);
    }
