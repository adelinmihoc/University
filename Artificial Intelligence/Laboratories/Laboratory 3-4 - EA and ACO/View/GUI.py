...
class GUI:
    ...
    def aco_button(self):
        ...
        d_map = self.map_service.get_map()
        sensors = [(1, 8), (2, 3), (4, 12), (6, 5), (8, 11)]
        aco_service = AcoService(d_map=d_map, sensors=sensors)
        sensors = aco_service.get_sensors()
        sensors_paths = aco_service.get_sensors_paths()

        starting_point = aco_service.get_starting_point()
        ...
        ant_energy = 60
        sol, energy_each_sensor = aco_service.solver(ant_energy)
        print(sol)
        print(sensors_paths)
        print(energy_each_sensor)
        ...
        for i in range(len(sol) - 1):
            ...
            left_flag = False
            right_flag = False
            for j in range(1, energy_each_sensor[sensors[sol[i]]]):
                x = sensors[sol[i]][0]
                y = sensors[sol[i]][1]
                x = x - j
                if x >= 0:
                    if self.map_service.get_map().surface[x][y] == 0 and up_flag is False:
                        self.screen.blit(brick, (y * 20, x * 20))
                    else:
                        up_flag = True
                x = sensors[sol[i]][0]
                ...
                x = x + j
                if x <= 19:
                    if self.map_service.get_map().surface[x][y] == 0 and down_flag is False:
                        self.screen.blit(brick, (y * 20, x * 20))
                    else:
                        down_flag = True
                x = sensors[sol[i]][0]
                ...
                y = y - j
                if y >= 0:
                    if self.map_service.get_map().surface[x][y] == 0 and left_flag is False:
                        self.screen.blit(brick, (y * 20, x * 20))
                    else:
                        left_flag = True
                x = sensors[sol[i]][0]
                ...
                y = y + j
                if y <= 19:
                    if self.map_service.get_map().surface[x][y] == 0 and right_flag is False:
                        self.screen.blit(brick, (y * 20, x * 20))
                    else:
                        right_flag = True
            self.screen.blit(sensor1, (sensors[sol[i]][1] * 20, sensors[sol[i]][0] * 20))

            current_path = sensors_paths[(sensors[sol[i]], sensors[sol[i + 1]])]
            for x in current_path:
                self.screen.blit(drona, (x[1] * 20, x[0] * 20))
                pygame.display.flip()
                self.screen.blit(brick, (x[1] * 20, x[0] * 20))
                time.sleep(0.5)

        sensors_powers = energy_each_sensor
        sensor = sensors[sol[-1]]
        up_flag = False
        down_flag = False
        left_flag = False
        right_flag = False
        for i in range(1, sensors_powers[sensor]):
            x = sensor[0]
            y = sensor[1]
            x = x - i
            if x >= 0:
