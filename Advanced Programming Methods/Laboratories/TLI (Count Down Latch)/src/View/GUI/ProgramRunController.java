...
public class ProgramRunController implements Initializable {
    ...
    @FXML
    public TableView<HashMap.Entry<Integer, Value>> LatchTable_tableView;
    @FXML
    public TableColumn<HashMap.Entry<Integer, Value>, String> latchTableLocation_tablCol;
    @FXML
    public TableColumn<HashMap.Entry<Integer, Value>, String> latchTableVal_tablCol;
    ...
    public void setLatchTable() {
        ObservableList<Map.Entry<Integer, Value>> latchTableList = FXCollections.observableArrayList();
        latchTableLocation_tablCol.setCellValueFactory(cellData->new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getKey())));
        latchTableVal_tablCol.setCellValueFactory(cellData->new ReadOnlyStringWrapper(Value.toString(cellData.getValue().getValue())));
        latchTableList.addAll(controller.getRepository().getProgramsList().get(0).getLatchTable().getContent().entrySet());
        LatchTable_tableView.setItems(latchTableList);
        LatchTable_tableView.refresh();
    }
