...
void MAIN_GUI::init_main_gui() {
    ...
    this->admin_file_path_edit->setText("lab9/dataout.txt");
    this->user_file_path_edit->setFont(f);
    this->user_file_path_edit->setText("lab9/csvfile.csv");

    // add rows to form layout
    if (this->controller.get_admin_repo_type() == "file") {
...
void MAIN_GUI::set_admin_file_path_button_handler() {
    ...
    if (test.is_open()) {
        ...
        QTimer::singleShot(1500, this, SLOT(update_buttons_colour()));
    }
    else {
...
void MAIN_GUI::update_buttons_colour() {
    this->admin_file_path_edit->setStyleSheet("background-color: solid white");
    this->user_file_path_edit->setStyleSheet("background-color:solid white");
}
