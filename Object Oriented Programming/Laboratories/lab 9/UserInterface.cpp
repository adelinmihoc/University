...
void RunicAltar::lightRunicAltar() {
    while (true){
        ...
        if (command == "myfileLocation") {
            try {
                setUserFilePath(parametersLine);
            }
...
void RunicAltar::showEnchantmentsMage() {
    ...
    std::cout << "'mode B' to enter in Apprentice mode.\n";
    std::cout << "'fileLocation /path/' to set the path to the Archmage file.\n";
...
void RunicAltar::showEnchantmentsApprentice() {
    ...
    std::cout << "'mode A' to enter in Archmage mode.\n";
    std::cout << "'myfileLocation /path/' to set the path to the Apprentice file.\n";
    std::cout << "'exit' to exit this mode.\n\n";
}
...
void RunicAltar::addGuardianStatue(const std::string& parametersLine) {
    ...
    try {
        ...
        throw UIError("Age is not a number!\n");
    }
...
void RunicAltar::deleteGuardianStatue(const std::string& parametersLine) {
    ...
    if (parameters.size() != 1) {
        throw UIError("Invalid number of parameters!\nEnchantment should have 1 parameter!\n");
    }
    std::string powerWordName = parameters[0];
...
void RunicAltar::listAllGuardianStatues(const std::string& parametersLine) {
    ...
    if (!parameters.empty()) {
        throw UIError("Invalid number of parameters!\nEnchantment shouldn't parameters!\n");
    }
...
void RunicAltar::listAllStatuesByMaterialAndAge(const std::string& parametersLine) {
    ...
    if (parameters.size() != 2) {
        throw UIError("Invalid number of parameters!\nEnchantment should have 2 parameters!\n");
    }
...
void RunicAltar::setUserFilePath(const std::string &parametersLine) {
    this->enchantment.setUserFilePath(const_cast<std::string & >(parametersLine));
    this->enchantment.setUserFileType();
}
