...
private:
	...
	void nextGuardStatue(std::string parametersLine);


	//Saves the current Guardian Statue
	void saveGuardianStatue(std::string parametersLine);


	//Lists all apprentice's stautes
	void listAllApprenticeStatues(std::string parametersLine);


	//List all apprentice's statues by material and age
	void listAllStatuesByMaterialAndAge(std::string parametersLine);
};