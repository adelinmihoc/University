...
void Tests::testRepository() {
    ...
    assert(encl.getSize() == 1);
    encl.updateStatue(g1);
    std::vector<GuardianStatue> gs = encl.getAllStatues();
    assert(gs.size() == 1);
    try {
...
void Tests::testIterator() {
    ...
    it.next();
    it.next();
    std::vector<GuardianStatue> dv = it.getAll();
    assert(dv.size() == 2);
}
...
void Tests::testController() {
    ...
    ArchmageEncyclopedia encl = ArchmageEncyclopedia();
    ...
    std::vector<GuardianStatue> dv = ench.getAllGuardianStatues();
    assert(dv.size() == 1);
    std::vector<GuardianStatue> dv2 = ench.getAllApprenticeGuardianStatues();
    assert(dv2.empty());
    ...
    std::vector<GuardianStatue> dv3 = ench.getAllApprenticesStatuesByMaterial("metal");
    std::vector<GuardianStatue> dv4 = ench.getAllApprenticesStatuesByMaterial("stone");
    std::vector<GuardianStatue> dv5 = ench.getAllApprenticeStatuesByMaterialAndAge("stone", 1000);
    assert(dv5.size() == 1);*/
}
